...
public class Day04 {
    ...
    public static void main(String[] args) {
        String input = "[1518-11-13 00:04] Guard #2411 begins shift\n" +
        ...
        int timeWhenGuardStarsSleep = -1;
        int timeWhenGuardWakesUp = -1;
        Integer id = -1;

        HashMap<Integer, int[]> guardSchedule = new HashMap<Integer, int[]>();

        for (String guardEntry : inputArray) {
            int time = -1;

            Pattern patternTime = Pattern.compile(":(\\d+)].+");
            Matcher matcherTime = patternTime.matcher(guardEntry);


            while (matcherTime.find()) {
                time = Integer.valueOf(matcherTime.group(1));
            }

            if (guardEntry.contains("#")) {
                Pattern patternId = Pattern.compile("#(\\d+)");
                Matcher matcherId = patternId.matcher(guardEntry);

                while (matcherId.find()) {
                    id = Integer.valueOf(matcherId.group(1));
                }

                timeWhenGuardStarsSleep = -1;
                timeWhenGuardWakesUp = -1;
            } else if (guardEntry.contains("falls")) {
                timeWhenGuardStarsSleep = time;
            } else if (guardEntry.contains("wakes")) {
                timeWhenGuardWakesUp = time;
            }

            if (timeWhenGuardStarsSleep != -1 && timeWhenGuardWakesUp != -1) {
                fillTimeSchedule(guardSchedule, id, timeWhenGuardStarsSleep, timeWhenGuardWakesUp);
            }
        }
        System.out.println("---Part one---");
        findMostSleepingGuard(guardSchedule);
        System.out.println("---Part two---");
        findGuardWithMinuteThatHeSleepsMost(guardSchedule);
    }

    public static HashMap<Integer, int[]> fillTimeSchedule(HashMap<Integer, int[]> guardSchedule, int id, int timeWhenGuardStarsSleep, int timeWhenGuardWakesUp) {
        int[] timeSchedule = new int[60];

        if (guardSchedule.containsKey(id)) {
            timeSchedule = guardSchedule.get(id);
        }

        for (int i = timeWhenGuardStarsSleep; i < timeWhenGuardWakesUp; i++) {
            timeSchedule[i]++;
        }
        guardSchedule.put(id, timeSchedule);

        return guardSchedule;
    }

    public static void findMostSleepingGuard(HashMap<Integer, int[]> guardSchedule) {
        int highestTimeAsleep = 0;
        int id = 0;
        int mostSleepingMinute = 0;
        int minuteWhenGuardSleepsMost = 0;


        for (Integer guard : guardSchedule.keySet()) {
            int[] timeSchedule = new int[60];
            timeSchedule = guardSchedule.get(guard);
            int timeSleeping = 0;
            int mostSleepingMinutes = 0;

            for (int minute = 0; minute < timeSchedule.length; minute++) {
                timeSleeping += timeSchedule[minute];
                if (timeSchedule[minute] > mostSleepingMinutes) {
                    mostSleepingMinutes = timeSchedule[minute];
                    mostSleepingMinute = minute;
                }
            }
            if (timeSleeping > highestTimeAsleep) {
                id = guard;
                highestTimeAsleep = timeSleeping;
                minuteWhenGuardSleepsMost = mostSleepingMinute;
            }
        }
        System.out.println("ID of guard sleeping most: " + id);
        System.out.println("Minutes asleep: " + highestTimeAsleep);
        System.out.println("Minute when guard sleep most: " + minuteWhenGuardSleepsMost);
        System.out.println("Answer: " + (id * minuteWhenGuardSleepsMost));
    }

    public static void findGuardWithMinuteThatHeSleepsMost(HashMap<Integer, int[]> guardSchedule) {
        int id = 0;
        int highestSleepingMinute = 0;
        int hignesNumberOfminutesSleeping = 0;
        ...
        for (Integer guard : guardSchedule.keySet()) {
            ...
            for (int minute = 0; minute < timeSchedule.length; minute++) {
                if (timeSchedule[minute] > hignesNumberOfminutesSleeping) {
                    id = guard;
                    hignesNumberOfminutesSleeping = timeSchedule[minute];
                    highestSleepingMinute = minute;
                }
            }
        }
        System.out.println("ID of guard that has highest sleeping minute: " + id);
        System.out.println("Highest sleeping minute: " + highestSleepingMinute);
        System.out.println("Number of minutes asleep in specific minute: " + hignesNumberOfminutesSleeping);
        System.out.println("Answer: " + (id * highestSleepingMinute));
    }
}
